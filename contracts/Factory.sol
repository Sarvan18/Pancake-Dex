...
contract pancakePair is IpancakePair, pancakeERC20 {
    ...
    function _update(
        ...
        uint32 timeElapsed = blockTimestamp >= blockTimestampLast
            ...
            : (4294967295 - blockTimestampLast) + blockTimestamp + 1;
    ...
    function burnWithLp(
        ...
        address _to,
        address _WETH
    ) external lock returns (uint amountToken, uint amountETH) {
        (uint112 _reserve0, uint112 _reserve1, ) = getReserves(); // gas savings
    ...
    function emergency(IERC20 token, address _to, uint256 _amount) external {
...
contract pancakeFactory is IpancakeFactory {
    ...
    function updateTokens(address _token, address _busd) external {
        ...
        if (token0 == token || token1 == token) {
            if (token0 == BUSD || token1 == BUSD) {
                require(
                    ...
                    "Time left for creating the pair"
                );
            }
        }
